...
void fs421_free_block(struct inode *inode, unsigned long block, unsigned size_log2)
{
  ...
  int i;
  /* Error out if block is not in legal range for disk */
  if(block < sbi->s_firstdatazone || block >= sbi->s_nzones) {
    printk("ERROR: Trying to free block not in data zone.\n");
    return ;
  }

  /* Check if block is properly aligned for binary size */
  if(block & ((1 << level) - 1) != 0) {
    printk("ERROR: BLOCK NOT PROPERLY ALIGNED\n");
    return ;
  }

  level = size_log2;
  zchunk = block - sbi->s_firstdatazone; /* get zone from physical disk offset */

  spin_lock(&bitmap_lock);  /* lEAVE THIS ALONE! */
  
  get_bh_and_bit(sb, level, zchunk, &bh, &bit);

  /* Check if bit in level is already free.
   * If not, free bit at level & mark buffer as changed
   */
  for(i = level; i < sbi->s_zmap_nlevels; i++) {
    ...
    /* Check if bit is already free. If not, free it & mark buffer as changed */
    if(test_bit_le(bit, bh->b_data) ==  0) {
      return ;
    }

    /* Then check buddy to determine if coalescing is necessary */
    if(test_bit_le(bbit, bh->b_data) == 0) {  /* Coalescing is necessary */
      printk("BUDDY BIT IS FREE-->SETTING BIT & BUDDY. COALESCING NECESSARY\n");
...
int fs421_new_block(struct inode * inode, unsigned size_log2)
{
  ...
  for(i = level; i < sbi->s_zmap_nlevels; i++) {
    level_size = sbi->s_zmap_offset[i+1] - sbi->s_zmap_offset[i];
    get_bh_and_bit(sb, i, zchunk, &bh, &bit);
    word = (__u32*)bh->b_data + bit/32;
    
    empty_bit_index = find_first_zero_bit_le(word, level_size);  /* Find the first empty bit */
    
    /* Break out of loop when the first zero bit is found */
    if(empty_bit_index < level_size) {
      printk("EMPTY BIT FOUND: Level %d, Index %d\n", i, empty_bit_index);
      if(empty_bit_index >= sbi->s_nzones) {
	spin_unlock(&bitmap_lock);	
	return 0;
      }
      set_bit_le(empty_bit_index, word);
      mark_buffer_dirty(bh);
      break;
    }
    printk("ESCALATING: Level %d (Size: %d), Bit %u\n", i, level_size, bit);
  }

  /* Return error if level is too large */
  if(level >= sbi->s_zmap_nlevels) {
    spin_unlock(&bitmap_lock);
    return 0;
  }

  /* Empty bit is found at higher level --> split until at requested level */
  for( ; i > level; i--) {
    set_bit_le(empty_bit_index, word);
    mark_buffer_dirty(bh);
    printk("SET BIT: Level %d, Index %d\n", i, empty_bit_index);

    word = (__u32*)bh->b_data + sbi->s_zmap_offset[i-1]/32;

    empty_bit_index *= 2;
    /* Clear one of the children */
    clear_bit_le(empty_bit_index + 1, word);
    mark_buffer_dirty(bh);

    printk("CLEAR BIT: Level %d, Index %d\n", i, empty_bit_index + 1);
  }
  spin_unlock(&bitmap_lock);
  
  return sbi->s_firstdatazone + (empty_bit_index << level); /*empty_bit_index = bit*/ /*RETURN DISK-RELATIVE OFFSET OF CHUNK, OR 0 FOR ERROR;*/
}
