		       ...
		       struct buffer_head *bh, int create)
{
  ...
  if (level < 0)
    ...
    */
     ...
     */
    for(l = 0; l < sizeof(idata); l++) {
      if(idata[l] == 0 && l <= level) {
	idata[l] = fs421_new_block(inode, l);  /* Call function in bitmap.c */
	printk("itree.c:: create = %d; level = %d; idata[i] = %d\n", create, l, idata[l]);
      }
    }
	...
	fs421_free_block(inode, block, l);
	break;
      }
    }
  }
